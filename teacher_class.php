				...
				?>
						<li id="del<?php echo $id; ?>">
							...
							<div style="display: flex; justify-content: space-between; align-items: center;">
								...
								<input type="checkbox" onchange="update_assignment(<?php echo $row['class_id']; ?>, this)" <?php echo $row['has_assignment'] ? 'checked' : '' ?> />
							</div>
			...
			<script>
				function update_assignment(id, el) {
					let state
					if (el.checked) {
						state = 1
					} else {
					...
					ajax.open('POST', 'utils/assignment.php')
					ajax.setRequestHeader('Content-type', 'application/x-www-form-urlencoded')
					ajax.send(`state=${state}&class_id=${id}`)
				}
			</script>