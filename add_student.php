...
$query4 = ("select subject_id from teacher_class where teacher_class_id = '$class_id' and teacher_id='$teacher_id' ");
$row=$conn-> query($query4);
$totalRows=mysqli_num_rows($row);
$show1 =mysqli_fetch_assoc($row);
$subject_id=$show1['subject_id'];


	if($Add == 'Add'){
	
	
	mysqli_query($conn,"insert into teacher_class_student (student_id,teacher_class_id,teacher_id, subject_id) values('$id','$class_id','$teacher_id','$subject_id') ")or die(mysqli_error());
